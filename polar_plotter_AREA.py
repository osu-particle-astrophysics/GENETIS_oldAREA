...
## AREA-specific Edits: Bryan Reynolds (reynolds.886@osu.edu)
## Date: 11/2/22
...
#Define ranges of lines to be read for azimuthal gains at the given freq_num
#There are 2668 lines for each frequency block (including headers)
#And azimuthal gain values are lines 4-40 counting from the header of each freq
#So read lines between (2668*freq_num)+4 and (2668*freq_num)+40
azimuthGainStart = ((g.freq_num-1)*2668)+4
azimuthGainEnd = ((g.freq_num-1)*2668)+40

#print(azimuthGainStart)
#print(azimuthGainEnd)
...
for i in range(g.initial_antenna, g.final_antenna+1):
	...
	with open(g.source1 + "/child_" + str(i) + ".txt", "r") as f:
		...
		for m, row in enumerate(uan_read):
			## skip the header
                        ## for AREA: counting from frequency header, rows 4-40 for azimuthal gain 
			#if m >= 4 and m <= 40:
                        if m >= azimuthGainStart and m <= azimuthGainEnd:
