...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    float sum_val[column_no][parent_no] = {0};
    float mean_val[column_no][parent_no] = {0};
   ...
   for (int t=0;t<parent_no;t++)
   {
       ...
       for (int n=0;n<column_no;n++)
       {
           for (int u=0;u<row_no;u++)
           {
               sum_val[n][t]+=fabs(gain_input[t][u][n]);
           }
           mean_val[n][t]=sum_val[n][t]/(column_no*row_no);
       }
    }
    ...
    /** ROULETTE WHEEL SELECTION **/
        for (int k=0;k<roul_no;k++) //Each individual in the population
        {
            ...
            else
            {
                ...
                std::normal_distribution <float> distribution(mean_val[i][k],mean_val[i][k]/10+mean_val[i][k]*temp/1000);
                            for(int j=0;j<row_no;j++)
                            {
	...
	int mat_size=ceil(0.07*parent_no); int max_neu=-1; int max_neu2=-1; //Size of the tournament = "mat_size"
        vector<int> mat_loc(mat_size, 0);

        for (int n=0;n<mat_size;n++)
        {
            mat_loc[n]=rand()%parent_no;
            if (Veff[mat_loc[n]]>Veff[max_neu])
            {
		max_neu=mat_loc[n];
	    }
	    mat_loc[n]=rand()%parent_no;
	    if (Veff[mat_loc[n]]>Veff[max_neu2]);
	    {
		max_neu2=mat_loc[n];
	    }
        }
    ...
    for(int i = 0; i < child_no; i++) {
    	...
    	for(int j = 0; j < 60; j++) {
    		...
    		// Correct negative gains here
    		float minGain = evalGain(gain_output[i][j]);
    		cout << "old minGain: " << minGain;
    		...
    		if(minGain < 0)
    		{
    			...
    			{
    				if(k == 0){
    					gain_output[i][j][k] = 1/(1-minGain)*( gain_output[i][j][k] - (2*sqrt(PI)*minGain) );

    				} else {
    					gain_output[i][j][k] = 1/(1-minGain)*gain_output[i][j][k];
    				}
    			}
    		}

    		minGain = evalGain(gain_output[i][j]);
    		cout << "\tnew minGain: " << minGain << endl; 
    	}
        ...
        for (int p = 0; p < 72; p++)
        {
            ...
            {   
                ...
                if(dataType == DataType::poly) {
                    ...
                    gain[f][p][th] = gainVector[0][0]*pow(theta[th]*PI/180.0, 0) + 
                                     gainVector[0][1]*pow(theta[th]*PI/180.0, 1) + 
                                     gainVector[0][2]*pow(theta[th]*PI/180.0, 2) + 
                                     gainVector[0][3]*pow(theta[th]*PI/180.0, 3) + 
                                     gainVector[0][4]*pow(theta[th]*PI/180.0, 4) + 
                                     gainVector[0][5]*pow(theta[th]*PI/180.0, 5) + 
                                     gainVector[0][6]*pow(theta[th]*PI/180.0, 6) + 
                                     gainVector[0][7]*pow(theta[th]*PI/180.0, 7) + 
                                     gainVector[0][8]*pow(theta[th]*PI/180.0, 8) + 
                                     gainVector[0][9]*pow(theta[th]*PI/180.0, 9) + 
                                     gainVector[0][10]*pow(theta[th]*PI/180.0, 10) +  
                                     gainVector[0][11]*pow(theta[th]*PI/180.0, 11) + 
                                     gainVector[0][12]*pow(theta[th]*PI/180.0, 12);
                    ...
                    phase[f][p][th] = fmod((phaseVector[f][0]*pow(theta[th]*PI/180.0f, 0) + 
                                  phaseVector[0][1]*pow(theta[th]*PI/180.0f, 1) + 
                                  phaseVector[0][2]*pow(theta[th]*PI/180.0f, 2) + 
                                  phaseVector[0][3]*pow(theta[th]*PI/180.0f, 3) + 
                                  phaseVector[0][4]*pow(theta[th]*PI/180.0f, 4) + 
                                  phaseVector[0][5]*pow(theta[th]*PI/180.0f, 5) + 
                                  phaseVector[0][6]*pow(theta[th]*PI/180.0f, 6) + 
                                  phaseVector[0][7]*pow(theta[th]*PI/180.0f, 7) + 
                                  phaseVector[0][8]*pow(theta[th]*PI/180.0f, 8) + 
                                  phaseVector[0][9]*pow(theta[th]*PI/180.0f, 9) + 
                                  phaseVector[0][10]*pow(theta[th]*PI/180.0f, 10) + 
                                  phaseVector[0][11]*pow(theta[th]*PI/180.0f, 11) + 
                                  phaseVector[0][12]*pow(theta[th]*PI/180.0f, 12))*180/PI, 180);
                ...
                // Populate the gain and phase vectors for a spherical harmonic fit
                    ...
                    gain[f][p][th] = gainVector[0][0]*(1/2.0)*(1/sqrt(PI)) +
                                     gainVector[0][1]*(1/2.0)*sqrt(3/PI)*cos(theta[th]*PI/180.0f) +
                                     gainVector[0][2]*(1/4.0)*sqrt(5/PI)*(3*pow(cos(theta[th]*PI/180.0f), 2)- 1) +
                                     gainVector[0][3]*(1/4.0)*sqrt(7/PI)*(5*pow(cos(theta[th]*PI/180.0f),3)- 3*cos(theta[th]*PI/180.0f)) +
                                     gainVector[0][4]*(3/16.0)*sqrt(1/PI)*(35*pow(cos(theta[th]*PI/180.0f),4) - 30*pow(cos(theta[th]*PI/180.0f),2)+3) +
                                     gainVector[0][5]*(1/16.0)*sqrt(11/PI)*(15*cos(theta[th]*PI/180.0f) - 70*pow(cos(theta[th]*PI/180.0f),3)+63*pow(cos(theta[th]*PI/180.0f),5)) +
                                     gainVector[0][6]*(1/32.0)*sqrt(13/PI)*(-5 + 105*pow(cos(theta[th]*PI/180.0f),2)-315*pow(cos(theta[th]*PI/180.0f),4) + 231*pow(cos(theta[th]*PI/180.0f),6)) +
                                     gainVector[0][7]*(1/32.0)*sqrt(15/PI)*(-35*cos(theta[th]*PI/180.0f)+ 315*pow(cos(theta[th]*PI/180.0f),3) -693*pow(cos(theta[th]*PI/180.0f),5) + 429*pow(cos(theta[th]*PI/180.0f),7)) +
                                     gainVector[0][8]*(1/256.0)*sqrt(17/PI)*(35 - 1260*pow(cos(theta[th]*PI/180.0f),2) + 6930*pow(cos(theta[th]*PI/180.0f),4) - 12012*pow(cos(theta[th]*PI/180.0f),6) + 6435*pow((cos(theta[th]*PI/180.0f)),8)) +
                                     gainVector[0][9]*(1/256.0)*sqrt(19/PI)*(315*cos(theta[th]*PI/180.0f)- 4620*pow(cos(theta[th]*PI/180.0f),3) + 18018*pow(cos(theta[th]*PI/180.0f),5) - 25740*pow(cos(theta[th]*PI/180.0f),7) + 12155*pow((cos(theta[th]*PI/180.0f)),9)) +
                                     gainVector[0][10]*(1/512.0)*sqrt(21/PI)*(-63 +3465*pow(cos(theta[th]*PI/180.0f),2) - 30030*pow(cos(theta[th]*PI/180.0f),4) + 90090*pow(cos(theta[th]*PI/180.0f),6) -109395*pow((cos(theta[th]*PI/180.0f)),8)+46189*pow(cos(theta[th]*PI/180.0f),10)) +  
                                     gainVector[0][11]*(1/512.0)*sqrt(23/PI)*(-693*pow(cos(theta[th]*PI/180.0f),1) +15015*pow(cos(theta[th]*PI/180.0f),3) - 90090*pow(cos(theta[th]*PI/180.0f),5) +218790*pow((cos(theta[th]*PI/180.0f)),7)-230945*pow(cos(theta[th]*PI/180.0f),9)+88179*pow(cos(theta[th]*PI/180.0f),11)) +
                                     gainVector[0][12]*(1/2048.0)*sqrt(25/PI)*(231 -18018*pow(cos(theta[th]*PI/180.0f),2) +225225*pow(cos(theta[th]*PI/180.0f),4) - 1021020*pow(cos(theta[th]*PI/180.0f),6) +2078505*pow((cos(theta[th]*PI/180.0f)),8)-1939938*pow(cos(theta[th]*PI/180.0f),10)+676039*pow(cos(theta[th]*PI/180.0f),12));
                    ...
                    phase[f][p][th] = fmod(gainVector[0][0]*(1/2.0)*(1/sqrt(PI)) +
                                     gainVector[0][1]*(1/2.0)*sqrt(3/PI)*cos(theta[th]*PI/180.0f) +
                                     gainVector[0][2]*(1/4.0)*sqrt(5/PI)*(3*pow(cos(theta[th]*PI/180.0f), 2) - 1) +
                                     gainVector[0][3]*(1/4.0)*sqrt(7/PI)*(5*pow(cos(theta[th]*PI/180.0f),3)- 3*cos(theta[th]*PI/180.0f)) +
                                     gainVector[0][4]*(3/16.0)*sqrt(1/PI)*(35*pow(cos(theta[th]*PI/180.0f),4) - 30*pow(cos(theta[th]*PI/180.0f),2)+3) +
                                     ...
                                     gainVector[0][10]*(1/512.0)*sqrt(21/PI)*(-63 +3465*pow(cos(theta[th]*PI/180.0f),2) - 30030*pow(cos(theta[th]*PI/180.0f),4) + 90090*pow(cos(theta[th]*PI/180.0f),6) -109395*pow((cos(theta[th]*PI/180.0f)),8)+46189*pow(cos(theta[th]*PI/180.0f),10)) +  
                                     gainVector[0][11]*(1/512.0)*sqrt(23/PI)*(-693*pow(cos(theta[th]*PI/180.0f),1) +15015*pow(cos(theta[th]*PI/180.0f),3) - 90090*pow(cos(theta[th]*PI/180.0f),5) +218790*pow((cos(theta[th]*PI/180.0f)),7)-230945*pow(cos(theta[th]*PI/180.0f),9)+88179*pow(cos(theta[th]*PI/180.0f),11)) +
                                     gainVector[0][12]*(1/2048.0)*sqrt(25/PI)*(231 -18018*pow(cos(theta[th]*PI/180.0f),2) +225225*pow(cos(theta[th]*PI/180.0f),4) - 1021020*pow(cos(theta[th]*PI/180.0f),6) +2078505*pow((cos(theta[th]*PI/180.0f)),8)-1939938*pow(cos(theta[th]*PI/180.0f),10)+676039*pow(cos(theta[th]*PI/180.0f),12))*180/PI,180);
