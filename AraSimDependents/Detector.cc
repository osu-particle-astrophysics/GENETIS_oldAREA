...
Detector::Detector(Settings *settings1, IceModel *icesurface, string setupfile, string antennaVpolfile) {
...
//        cout<<"total station_count : "<<station_count<<endl;
	  ...
	  //SetupInstalledStations();        
        ...
        ReadVgain(antennaVpolfile);
        // test read H-pol gain file!!
        ReadHgain(antennaVpolfile);
        // read filter file!!
...
inline void Detector::ReadVgain(string filename) {
    ...
    cout<<"Vpol File is : " << filename << endl;
            ...
            for (int i=0; i<freq_step; i++) {
                ...
                if ( line.substr(0, line.find_first_of(":")) == "freq ") {
                    ...
                    for (int j=0; j<ang_step; j++) {
                        ...
                        Vgain[i][j] = atof( line.substr( 25, 33 ).c_str() );  // read gain (not dB)
                        Vphase[i][j] = atof( line.substr( 38 ).c_str() );  // read phase
            	       // Vgain[i][j] = 0.0;
		       // Vphase[i][j] = 0.0;
		       // cout << filename << endl;                                    
                      //  cout<<"Freq: "<<Freq[i]<<", Ang. Step :" << j <<", VGain : "<<Vgain[i][j]<<", VPhase : "<<Vphase[i][j]<<endl;
            ...
            for (int i=0; i<freq_step; i++) {
                ...
                if ( line.substr(0, line.find_first_of(":")) == "freq ") {
                    ...
                    for (int j=0; j<ang_step; j++) {
                        ...
                        Hphase[i][j] = atof( line.substr( 38 ).c_str() );  // read gain (not dB)
...
void Detector::SetupInstalledStations(){
    ...
    int number_of_installed_stations = 2;
